...
import com.gu.pandomainauth.internal.planning.AuthPersistenceStatus
...
class NewlyOAuthedUserHandlerTest extends AnyFreeSpec with Matchers with OptionValues with Inside {
  val initialAuthedUser = AuthenticatedUser(User("John", "Smith", "", None), "composer", Set.empty, Instant.now().plusSeconds(1), true)
  ...
  "NewlyAuthenticatedUserHandler" - {
    "ensure that 'authenticatedIn' is updated once the user has been authorised (not just authenticated) by that system" in {
      ...
      initialAuthedUser.authenticatedIn shouldBe empty // authorisation has not happened yet

      val plan =
        handler.planFor(initialAuthedUser, priorAuth = AuthPersistenceStatus(NotAuthenticated, Set.empty), returnUrl = examplePageRequestUrl)
