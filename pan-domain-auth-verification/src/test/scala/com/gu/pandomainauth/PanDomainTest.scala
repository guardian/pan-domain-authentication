...
class PanDomainTest extends FreeSpec with Matchers with Inside {
  ...
  "authStatus" - {
    val authUser = AuthenticatedUser(User("test", "user", "test.user@example.com", None, Some("123456")), "testsuite", Set("testsuite"), new Date().getTime + 86400, multiFactor = true)
  ...
  "guardianValidation" - {
    val validUser = AuthenticatedUser(User("example", "user", "example@guardian.co.uk", None, Some("123456")), "tests", Set("tests"), new Date().getTime + 86400, multiFactor = true)
